...
import com.lbx.cloudgallery.model.enums.SpaceRoleEnum;
import com.lbx.cloudgallery.model.enums.SpaceTypeEnum;
...
import com.lbx.cloudgallery.service.SpaceUserService;
...
public class SpaceServiceImpl extends ServiceImpl<SpaceMapper, Space>
    ...
    public long addSpace(SpaceAddRequest spaceAddRequest, User loginUser) {
        ...
        synchronized (lock) {
            Long newSpaceId = transactionTemplate.execute(status -> {
                //判断是否已有空间
                boolean exists =
                        this.lambdaQuery().eq(Space::getUserId, userId).eq(Space::getSpaceType,
                                space.getSpaceType()).exists();
                //如果已有空间，就不能再创建
    ...
    public void validSpace(Space space, boolean add) {
        ...
        //创建时校验
        if (add) {
            ThrowUtils.throwIf(StrUtil.isBlank(spaceName), ErrorCode.PARAMS_ERROR, "名称不能为空");
